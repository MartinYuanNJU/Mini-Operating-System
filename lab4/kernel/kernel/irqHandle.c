...
void syscallSemWait(struct StackFrame *sf) {
	...
	if(sem[i].value<0)
	{
		...
		pcb[current].state=STATE_BLOCKED; //block the current process
...
void syscallSemPost(struct StackFrame *sf) {
	...
	if(sem[i].value<=0)
	{
		...
		struct ListHead *wakeup=sem[i].pcb.next;
		struct ProcessTable *processtable=(struct ProcessTable *)((uint32_t)(wakeup) - (uint32_t)(&((struct ProcessTable *)0)->blocked)); //get the head address of the waking-up pcb
		processtable->sleepTime=0;
		processtable->state=STATE_RUNNABLE;

		struct ListHead *wakeupnext=wakeup->next;
		sem[i].pcb.next=wakeupnext;
		wakeupnext->prev=&(sem[i].pcb);
	}
	pcb[current].regs.eax=0; //set return value
	return;
}
...
void syscallSemDestroy(struct StackFrame *sf) {
	...
	if(sem[i].pcb.next==&(sem[i].pcb)) //there doesn't exist any process blocked on this semaphore
	{
		sem[i].value=0;
		sem[i].state=0;
		sem[i].pcb.prev=sem[i].pcb.next=&(sem[i].pcb);
		pcb[current].regs.eax=0;
		return;
	}
	else //still has process blocked on this semaphore, cannot destroy
	{
		pcb[current].regs.eax=-1;
		return;
	}
